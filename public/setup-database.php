...
try {
    ...
    $sql = "CREATE DATABASE IF NOT EXISTS eventdb";
    if ($conn->query($sql) === TRUE) {
        echo "<p style='color: green;'>✓ Database 'eventdb' created or already exists</p>";
    } else {
    ...
    echo "<h3>Inserting Sample Events:</h3>";
    foreach ($sample_events as $sql) {
        if ($conn->query($sql) === TRUE) {
            echo "<p style='color: green;'>✓ Sample event inserted</p>";
        } else {
            echo "<p style='color: orange;'>⚠ Event already exists or error: " . $conn->error . "</p>";
        }
    }
    
    // Insert default admin
    $admin_sql = "INSERT INTO admins (username, email, password, full_name) VALUES 
    ...
    if ($conn->query($admin_sql) === TRUE) {
        ...
        echo "<p style='color: orange;'>⚠ Admin account already exists or error: " . $conn->error . "</p>";
    }
